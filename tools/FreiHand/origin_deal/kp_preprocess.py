...
description = """ 
        04-核心目标：实现图像与3D空间的视角对齐，支持手部/物体姿态估计的标准化预处理
        get_points_center_scale：计算点集的中心坐标和最大包围范围
        get_points_bbox：生成点集的边界框坐标（[x_min, y_min, x_max, y_max]）
        preprocess：基于边界框对图像进行仿射变换裁剪和缩放
        projectPoints：将3D点投影到2D图像空间（针孔相机模型）
        cross：计算两个三维向量的叉乘结果
        cal_rot_mat_by_vector：通过两个向量生成旋转矩阵（罗德里格斯公式）
        get_trans_mat_by_center_K：根据图像中心点生成相机视角变换矩阵
        trans_3d_by_center_K：将3D点集应用视角变换矩阵
        cal_perspective_mat：计算图像透视变换矩阵和适配的新相机参数
        trans2d_perspective：应用透视变换矩阵到2D关键点坐标
        get_2d3d_perspective_transform：主函数，生成2D/3D联合变换矩阵和新相机参数
"""
